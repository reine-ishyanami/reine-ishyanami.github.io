import{_ as a,r as i,o as l,c as t,a as e,b as c,d as r,e as n}from"./app-410d86ed.js";const d={},o=n('<h1 id="apache-bench接口压测工具的使用" tabindex="-1"><a class="header-anchor" href="#apache-bench接口压测工具的使用" aria-hidden="true">#</a> Apache Bench接口压测工具的使用</h1><h2 id="性能测试指标" tabindex="-1"><a class="header-anchor" href="#性能测试指标" aria-hidden="true">#</a> 性能测试指标</h2><p><strong>并发数（concurrent）</strong>: 某个时间范围内。同时在使用系统的用户个数。从实际场景来看，并发数就是同时使用该服务器接口的客户端数，这些客户端可能调用不同的API。严格意义上来说，并发数是指同时请求同一个API的用户个数。</p><p><strong>每秒查询数（QPS）</strong>: QPS是对一个特定的查询服务器在规定时间内所处理流量多少的衡量标准。QPS = 并发数 / 平均请求响应时间。QPS就是一秒内查询完成的次数。</p><p><strong>每秒事务数（TPS）</strong>: 一个事务是指客户端向服务器发送请求，然后服务器做出反应的过程。客户端在发送请求时开始计时，收到服务器响应后结束计时，以此来计算使用的时间和完成的事务个数。TPS就是一秒内处理事务完成的次数。</p><ol><li><p>TPS和QPS有什么区别？</p><blockquote><p>如果是对一个查询接口（单场景）压测，且这个接口内部不会再去请求其他接口，那么 TPS=QPS，否则，TPS≠QPS。如果是对多个接口（混合场景）压测，假设 N 个接口都是查询接口，且这个接口内部不会再去请求其他接口，QPS=N*TPS。</p></blockquote></li><li><p>QPS和并发数是怎么样的关系？</p><blockquote><p>QPS = 并发数 / 平均请求响应时间。比如我们有500个客户端，并发对API发起请求，平均的请求返回时间是200ms，那么我们可以算出该接口的QPS=500/0.2=2500。</p><p>在并发数设置过大时，API 同时要处理很多请求，会频繁切换上下文，而真正用于处理请求的时间变少，反而使得 QPS 会降低。并发数设置过大时，请求响应时间也会变长。API 会有一个合适的并发数，在该并发数下，API 的 QPS 可以达到最大，但该并发数不一定是最佳并发数，还要参考该并发数下的平均请求响应时间。</p></blockquote></li><li><p>哪些指标最能反应API的处理性能？</p><blockquote><p>衡量 API 性能的最主要指标是 QPS，但是在说明 QPS 时，需要指明是多少并发数下的 QPS，否则毫无意义，因为不同并发数下的 QPS 是不同的。举个例子，单用户 100 QPS 和 100 用户 100 QPS 是两个不同的概念，前者说明 API 可以在一秒内串行执行 100 个请求，而后者说明在并发数为 100 的情况下，API 可以在一秒内处理 100 个请求。当 QPS 相同时，并发数越大，说明 API 性能越好，并发处理能力越强。当然上文也提到了，做性能评估时还需参考该并发数下的平均请求响应时间，如果并发数大的但平均请求响应时间还是很稳定，那么说明该API性能越好。</p></blockquote></li></ol><h2 id="apache-bench使用" tabindex="-1"><a class="header-anchor" href="#apache-bench使用" aria-hidden="true">#</a> Apache Bench使用</h2>',7),p={href:"https://httpd.apache.org/",target:"_blank",rel:"noopener noreferrer"},v=n(`<ol><li><p>安装方法</p><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code><span class="token function">apt</span> <span class="token function">install</span> apache2-utils  <span class="token comment"># debian系</span>
yum <span class="token function">install</span> httpd-tools <span class="token parameter variable">-y</span>  <span class="token comment"># redhat系</span>
brew <span class="token function">install</span> httpd-tools  <span class="token comment"># macOS</span>
scoop <span class="token function">install</span> apache  <span class="token comment"># windows</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div></li><li><p>命令基本使用:</p><div class="language-bash line-numbers-mode" data-ext="sh"><pre class="language-bash"><code>ab <span class="token parameter variable">-t</span> <span class="token number">5</span> <span class="token parameter variable">-n</span> <span class="token number">100</span> <span class="token parameter variable">-c</span> <span class="token number">10</span> http://localhost:8080/text  <span class="token comment"># 5秒内，并发数为10，请求100次</span>
</code></pre><div class="line-numbers" aria-hidden="true"><div class="line-number"></div></div></div></li><li><p>输出内容解释</p><div class="language-text line-numbers-mode" data-ext="text"><pre class="language-text"><code>This is ApacheBench, Version 2.3 &lt;$Revision: 1903618 $&gt;
